...
class Simulation:
    ...
    def __init__(self, name, population = None, food_decrease = False, display = False, decrease_rate = 0.1, decrease_time = 10):
        self.name = name
        self.display = display
        if population:
            self.INITIAL_POPULATION_COMPOSITION = population
        self.FOOD_DECREASE = food_decrease
        self.FOOD_DECREASE_RATE = decrease_rate
        self.FOOD_DECREASE_TIME = decrease_time


    '''
    Draw a square representing an Object
    '''
    def draw(self, color, x, y, factor = 1):
        pg.draw.rect(self.screen, color, (x, y, factor*BLOCK_SIZE, factor*BLOCK_SIZE))

    '''
    Draw the environment and its contents
    '''
    def draw_environment(self, screen, env):
            ...
            for o in env:
                ...
                self.draw(color, draw_xy[0], draw_xy[1], factor)
    ...
    '''
    def sleep_and_reproduce(self, env):
        babies = []
        for o in env:
            if not isinstance(o, Organism): continue
            if o.food_found >= 2:
                babies.append(o.reproduce(self.DAY_ENERGY))
            o.food_found = 0
            o.energy = self.DAY_ENERGY
    ...
    '''
    def do_one_step(self, env, m):
        minutes_left = self.MINUTES_PER_DAY - m
    ...
    '''
    def run_simulation(self, num_days=None):
        if not self.display and not num_days:
            num_days = 100
        if self.display:
            pg.init()
        ...
        while run:
            ...
            self.do_one_step(env, time_keeper["minute"])
            time_keeper["minute"] += 1
            if time_keeper["minute"] >= self.MINUTES_PER_DAY:
                print("Day {} is over.".format(time_keeper["day"]))
                time_keeper["minute"] = 0
                time_keeper["day"] += 1
                self.kill(env)
                self.sleep_and_reproduce(env)

                if self.FOOD_DECREASE and time_keeper["day"]%self.FOOD_DECREASE_TIME == 0:
                    n_food = int(n_food*(1-self.FOOD_DECREASE_RATE))
                    print("decreased food to {}".format(n_food))
                if self.REMOVE_EXCESS_FOOD:
            ...
            if self.display:
                self.draw_environment(self.screen, env)
                pg.display.update()
                pg.time.wait(10)
                pg.event.pump()
    ...
    def start(self, num_days):
        stats = self.run_simulation(num_days)

        with open('./results/exp_{}_{}.pickle'.format(self.name, time.time()), 'wb') as handle:
